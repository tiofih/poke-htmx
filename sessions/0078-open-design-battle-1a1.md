...
>
> **S3 — alteração de critério na revisão S7 (2026-09-16):** C2 era "mini-arena + `rewards` no padrão `battle-results-desktop.html`" e C3 citava "os 3 protótipos". Como os dois protótipos de fim são **variantes alternativas** (não composição), a variante `res-top`/`state-title` do `battle-results-desktop.html` foi descartada e as células foram reescritas para o que de fato se entrega (`battle-end-states.html`). **Reaprovada pelo usuário em 2026-09-16** (reaprovação explícita do usuário, fora do loop S7, antes da validação M1).
