...
| 4 | `34b07b0` | Apaga `db/migrations/0036_add_user_state.sql`. **Idempotência provada (existência):** 3 execuções consecutivas de `rake db:setup` com `exit 0`; `to_regclass('user_state')` = NULL nos dois bancos (`pokedex` **e** `pokedex_test`) e as 5 tabelas (`team_pokemons`, `team_pokemon_progress`, `battles`, `wallet`, `inventory`) seguem **existentes** — intactas **quanto à existência, não quanto ao conteúdo**: as migrações pré-existentes `db/migrations/0003:4`/`0007:4` (`TRUNCATE team_pokemons CASCADE`) rodam em **toda** execução do setup (sonda do revisor: 1 linha → 0; 108 → 0 observado), apagando o time do banco de dev a cada setup — efeito **pré-existente** (não introduzido pela 0089), já anotado como **T2** no `TODO.md`. **Isso não invalida a remoção do `user_state`** (a tabela some de fato), mas validar manualmente no app exige **recriar o time**. |
