...
- **Passo 1 (C1):** bloco `:root` com tokens oklch (do §1.1 do `migration-guide.md`) adicionado no TOPO do `style.css`, após o cabeçalho e **antes** dos blocos existentes — nenhuma regra atual movida/reordenada. `test_juice_reduced_motion_disables` continua verde → `prefers-reduced-motion` segue **depois** das `animation: juice-*`.
- **Passo 2 (C2):** classes-núcleo (`.topnav`, `.btn`-family, `.container`/`.section`/`.stack`/`.row`/`.row-between`/`.card`, `.eyebrow`/`.lead`/`.meta`/`.num`/`.muted`) adicionadas **dentro** do bloco delimitado do design system, após o `:root`. Suíte 1005/4035 + lint 0.
- **Passo 3 (C3+C6):** `layout.erb` portado para o shell (topnav + `#content` + footer pagefoot), preservando: JS de `#global-loading`/scroll-restore, `<meta name="viewport">`, link sakura (B2) e **`@team_size`/`/6`** no badge. `body` mantém `page-list` em `/` além de `page-battle`/`page-history`. A CTA "Batalhar" vira `class="active"` em `/battle` e `btn btn-primary` nas demais páginas (preserva `battle_routes_test`/`pokemon_routes_test`). Suíte 1007/4050 + lint 0.
- **Passo 4 (C4+C5):** bloco do design system **não** referencia `sakura` nem usa `@import` (`test_design_system_does_not_depend_on_sakura`); `get "/style.css"` responde **200 `text/css`** (estático do `public/`, passa pelo `before`/`after` — `TestDatabase.setup!` cobre, sem stub). Suíte 1009/4058 + lint 0.
- **Passo 5 (G1/G2/G3):** suíte completa **1009 runs / 4058 assertions, 0 failures, 0 errors, 0 skips** (baseline 1003/3980 preservado + 6 testes novos de `design_system_test.rb`); `./scripts/lint` 0 offenses; `./scripts/check_docs` ok → docs consistentes; `./scripts/checar-sessao 0072` ok; `git diff -- Gemfile* db/ server.rb lib/` **vazio** (G2).
