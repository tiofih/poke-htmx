...
### Confirmados na implementação (fase 2, 2026-09-08)

- **Rollback do `pg` 1.6.3 (gotcha acima validado):** `connection.transaction` emite `ROLLBACK` ao sair por exceção e re-levanta; após o `rescue PG::UniqueViolation`, a conexão volta a `idle` e o retry **re-entra num `begin` limpo** — re-rodar `next_free_slot`/`duplicate?` do início funciona sem estado abortado. Validado no green do Passo 1 (C1/C2 com 6 e 2 threads).
- **Nomes de constraint confirmados:** os `PG::UniqueViolation` reportam exatamente `idx_team_pokemons_user_number` (`(user_id, number)`) e `idx_team_pokemons_user_slot` (`(user_id, slot)`). O filtro `result.error_field(PG::PG_DIAG_CONSTRAINT_NAME)` casa com esses nomes (retry só nas corridas; FK/NOT NULL/PK propagam).
- **RuboCop `Naming/VariableNumber`:** nomes de teste **não** podem conter dígitos (ex.: `no_500`). Manter o nome exato do critério (S1) exige `# rubocop:disable Naming/VariableNumber` inline na linha do `def` — padrão já usado no projeto (ex.: `Metrics/AbcSize`).
- **RuboCop `Lint/AmbiguousBlockAssociation`:** usar `rows.count { }` como argumento de outro método (`assert_equal 1, rows.count { }`) dispara a cop; atribuir a uma variável local (`matching = rows.count { }`) resolve.
- **Barreira com `Queue` + `Thread#value`:** `ready` + `go` (Queues) garantem que todas as threads fiquem prontas antes do release; `threads.each(&:value)` **re-lança** a exceção da thread no chamador — é o que torna `test_concurrent_adds_do_not_raise` red de verdade (qualquer `PG::UniqueViolation` derruba o teste).
