...
| 0036 | J1 — seleção inicial de time: fim do dropdown (lista clicável sprite+nome+Add, pool total 20/página + destaques "Iniciais") + tela de entrada da jornada (gate battle/mart/center até time de 6, marcador `user_state` + derivado do time) | Implementação | Concluída — passos 1–6 + ajuste S3 (só formas base via `base_form?`; suíte 610/1896, lint 0), aguardando revalidação do usuário |
