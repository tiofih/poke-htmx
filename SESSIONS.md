...
| 0090 | cta-oob — CTA "Batalhar" (gate + hint) via OOB nas rotas que mexem no time (T5 do `TODO.md`): partial `views/_cta_slot.erb` (`#cta-slot`, D1=A, padrão `nav-badge`), OOB `outerHTML` em `POST`/`DELETE /team`, `POST /team/heal`, `POST /journey/restart`, `POST /mart/buy`/`sell`; asserts novos nos arquivos de rota (D3); pill/0083 intacta (M1); `> Reprodução: manual`, `> E2E: nao` | Refinamento | refinada em 2026-09-21, fase 2 pendente |
