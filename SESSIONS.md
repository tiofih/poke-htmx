...
**Próxima sessão (2026-09-15):** **0088-projetil-slot-a-slot** — o projétil sai do slot do pokémon atacante e chega ao slot do pokémon alvo em x e y (progressive enhancement sobre o rail CSS da 0087, que passa a fallback). Status: **refinamento concluído / implementação pendente** — refinada em 2026-09-15 (D1a rota JS medido escrevendo `--fx-dy`; D2a exceção estreita ao RNF-01 para um handler inline em `views/layout.erb`; D3a índices `attacker_index`/`target_index` no strike entry + `from_slot`/`to_slot` com guarda + `data-from-slot`/`data-to-slot` nos dois caminhos de render). A 0088 carrega **exceção aprovada ao RNF-01** e **reabre as semânticas C1/C6 da 0087** (slot→slot), ambas já registradas na nota S3 de `sessions/0087-strike-fx-chegada-real.md` e em `REQUIREMENTS.md:499-503`. Próxima ação = **fase 2 (TDD), Passos 0–6** (`sessions/0088-projetil-slot-a-slot.md` §6); a implementação **para na fase 2** (suíte + lint verdes, Revisor S7 `Aprovado`) e a validação (fase 3) é do **usuário** — nada marcado `Concluída` antes.
...
| 0077 | open-design-home-1a1 — Resíduo open-design (1ª de 3, por tela): `home-team.html` → miolo `_center` (`heal-list`/`heal-item`) + `_mart` (`mart`/`mart-name`/`item-icon`/`price`/`tabs`) + `team_manage` (`stat-grid`/`mv-row`/`evo-row`/`equip-row`) + `_evolution_modal` (padrão `.modal`) + `pokemon_list_item` (`pcard-meta`/`name`) + `pokemon_detail` (`tag-row`/`stat-grid`/`evo-row`); fidelidade híbrida, backend thin listado (5 renders, sem migração), CSS ANTES da linha `fim` com delimitador `0077`; critérios C1–C4 → `test/home_residue_test.rb` (novo: `test_center_mart_inner`/`test_manage_evolution`/`test_catalog_detail`) + extensões cirúrgicas + 1 manual | Refinada | Refinada em 2026-09-09 (fase 2 pendente; `style.css` dirty ~1129+/1003- — conferir/stash antes de editar) |
| 0078 | open-design-battle-1a1 — Resíduo open-design (2ª de 3, por tela): `battle.html` + `battle-end-states.html` + `battle-results-desktop.html` (nunca portados) → fim de `battle.erb` (`res-screen`/`state-card`/mini-arena, 4 estados) + fim de `_fighter_panel`; fidelidade híbrida, `resolve`/`finish_effects`/`expose_battle_result` só leitura, CSS ANTES da linha `fim` com delimitador `0078`; critérios C1–C3 → `test/battle_end_states_test.rb` (novo: `test_battle_end_states`/`test_results_desktop`) + extensões cirúrgicas + 1 manual | Refinada | Refinada em 2026-09-09 (fase 2 pendente; `style.css` dirty ~1129+/1003- — conferir/stash antes de editar) |
| 0079 | open-design-history-1a1 — Resíduo open-design (3ª de 3, por tela): curadoria fina 1:1 do `history.html` sobre `history.erb` + `history_page.erb` (base 0075 pronta); fidelidade híbrida (literal onde sem legado), backend thin listado, CSS ANTES da linha `fim` com delimitador `0079`; critérios C1–C2 → `test/history_curation_test.rb` (novo: `test_history_matches_prototype`) + extensões cirúrgicas + 1 manual | Refinada | Refinada em 2026-09-09 (fase 2 pendente; `style.css` dirty ~1129+/1003- — conferir/stash antes de editar) |
...
| 0088 | projetil-slot-a-slot — projétil sai do slot do atacante e chega ao slot do alvo em x e y (D1a rota JS medido escrevendo `--fx-dy`; D2a exceção estreita e formal ao RNF-01 para um handler inline em `views/layout.erb`; D3a índices `attacker_index`/`target_index` no strike entry + `from_slot`/`to_slot` com guarda + `data-from-slot`/`data-to-slot` nos dois caminhos de render; D4a/D5 rail CSS da 0087 vira fallback, coluna única no-op; D6a e2e mede par ordenado; D7a teste de `target_strategy`; D8 reaberturas formais) — sem lib/polling/SSE, motor/economia/rotas intocados; S3 reabre C1/C6 da 0087 | Refinada | Refinada em 2026-09-15 (fase 2 pendente) |
