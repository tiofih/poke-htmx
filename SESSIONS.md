...
| 0078 | open-design-battle-1a1 — Resíduo open-design (2ª de 3, por tela): `battle.html` + `battle-end-states.html` + `battle-results-desktop.html` (nunca portados) → fim de `battle.erb` (`res-screen`/`state-card`/mini-arena, 4 estados) + fim de `_fighter_panel`; fidelidade híbrida, `resolve`/`finish_effects`/`expose_battle_result` só leitura, CSS ANTES da linha `fim` com delimitador `0078`; critérios C1–C3 → `test/battle_end_states_test.rb` (novo: `test_battle_end_states`/`test_results_desktop`) + extensões cirúrgicas + 1 manual | Aguarda validação | Implementada em 2026-09-09 (Passos 1–3 `9b421c4`/`924e5b6`/`4604ee9`, escopo verde) — **aguarda validação do usuário (fase 3)**; atualizado em 2026-09-15 |
| 0079 | open-design-history-1a1 — Resíduo open-design (3ª de 3, por tela): curadoria fina 1:1 do `history.html` sobre `history.erb` + `history_page.erb` (base 0075 pronta); fidelidade híbrida (literal onde sem legado), backend thin listado, CSS ANTES da linha `fim` com delimitador `0079`; critérios C1–C2 → `test/history_curation_test.rb` (novo: `test_history_matches_prototype`) + extensões cirúrgicas + 1 manual | Aguarda revisão | TDD concluído em 2026-09-09 (Passos 1–2 verdes no escopo, lint 0) — **aguarda Revisor (S7)** e depois validação do usuário (fase 3); atualizado em 2026-09-15 |
