...
**Próxima sessão (2026-09-16):** **nenhuma sessão agendada — candidato a refinamento (ainda sem arquivo em `sessions/` e sem linha na tabela, S5b):** a **0083-ui-polish** foi concluída e validada em 2026-09-16 (S2 C1–C4+G1–G3+M1 ok). O próximo passo é **um refinamento pendente de limpeza de dívida técnica** — (a) remover o estado morto `user_state` (escrita viva `server.rb:599` → `lib/journey_service.rb:23-28` → `lib/user_state_repository.rb:21-27`; leitura morta — `started?` sem callers; 6 arquivos de teste dependem da tabela; ~9 arquivos/~70 linhas + migração de drop) e (b) reparar os 4 e2e de `e2e/specs/battle-log.spec.ts:51,77,101,112` (troca de 4 linhas; o 450 **não** está stale — fonte única `lib/team_budget.rb:8`). Candidato extra anotado: **stubar as chamadas de PokeAPI** para o VCR parar de acumular episódios a cada run (o `PokeApiStub` do `test_helper` é o caminho; hoje o `:new_episodes` fez um cassette versionado crescer 7× num run). Outro agente está refinando isso e ajustará número/linha da tabela depois.

**Próxima sessão (2026-09-16):** **0089-limpeza-user-state-e2e** (refinada, **fase 2 pendente**) — dívida técnica em dois blocos numa sessão só (D1): **(A)** remover o estado morto `user_state` (D2 apagar tudo; D3 apagar a migração de criação `0036_add_user_state.sql` + criar `0037_drop_user_state.sql` com `DROP TABLE IF EXISTS`); escreve no mesmo commit os docs que afirmavam o contrário (D7: `GDD.md:27`, `REQUIREMENTS.md:638-643`, `docs/draft-backlog.md:109`, `docs/5F-decisoes-pendentes.md`); o gate **fica** derivado do time (`lib/journey_service.rb:8`). **(B)** reparar os 4 e2e vermelhos de `e2e/specs/battle-log.spec.ts:51,77,101,112` (D4) — trocar o bug do `buildTeamOfSix` pelo `buildBudgetTeam`; o orçamento 450 **não** está stale. **D5** (stub de PokeAPI para o VCR parar de acumular episódios) fica **anotado e dimensionado no draft, não refinado**. Critérios C1–C8 + G1–G3 com prova por critério (S1); Bloco A = Passos 1–5, Bloco B = Passo 6 isolado; `./scripts/checar-sessao 0089` e `./scripts/check_docs` verdes no refinamento; nenhuma suíte/lint/e2e rodada na fase 1.

**Próxima sessão (2026-09-16):** **nenhuma sessão agendada — próximo item da fila: `T5` do `TODO.md`, a refinar (ainda sem arquivo em `sessions/` e sem linha na tabela, S5b)** — a **0089-limpeza-user-state-e2e** foi **concluída e validada** pelo usuário em 2026-09-16 (S2 C1–C9+G1–G3+M1 ok, sem `nok`; Passos 1–8 `d57947c`…`85f48af` + ajustes S7 `8cdc045`…`e17aaf9`; suíte 1185/6332, lint 0; revisor `Aprovado` na rodada 3) e não há novo arquivo de sessão a abrir. O primeiro item é o **`T5`**: o **CTA "Batalhar" (gate + hint) do cabeçalho fica stale até um F5** fora do full reload — nos **dois** sentidos — porque a casca (`views/layout.erb:20-37`) não é re-renderizada por swap htmx e nenhuma rota que mexe no time emite o CTA em OOB (`server.rb:603-605` manda só `#team`/`#pokemon`/`#nav-badge`); é **achado pós-validação da 0083**, consequência aceita do escopo dela (que preservou o full reload), **não** falha de critério — correção recomendada no `TODO.md`: OOB do CTA/hint (padrão que o projeto já usa). Na sequência: `T3`/`T4` (achados `low` da rodada 3 da 0089) e `T1`/`T2` (flake order-dependente do `SeedScriptsTest` e `db:setup` destrutivo pré-existente).

**Próxima sessão (2026-09-21):** **0090-cta-oob** (refinada, **fase 2 pendente**) — T5 do `TODO.md`: o CTA "Batalhar" (gate + hint) do cabeçalho passa a atualizar via OOB nas rotas que mexem no time (`POST`/`DELETE /team`, `POST /team/heal`, `POST /journey/restart`, `POST /mart/buy|sell`), sem F5 nos dois sentidos. D1=A (partial `views/_cta_slot.erb` com `#cta-slot`, padrão `nav-badge`); D3=asserts nos arquivos de rota existentes; critérios C1–C5 + G1–G3 + M1 (pill/0083 intacta); `> Reprodução: manual`, `> E2E: nao`.

**Sessão 0091 (t3-t4-rodada3) CONCLUÍDA e validada em 2026-09-21** (PR #3 mergeado pelo
usuário — C1/C2 ok: reindent `<p class="rewards">` + assert exato de XP, T3/T4 do `TODO.md`
fechados). **Próxima sessão: 0092 — T1 + T2 do `TODO.md`** (flake do `SeedScriptsTest` +
`db:setup` destrutivo documentado/atenado), a refinar.

**Próxima sessão (2026-09-21):** **0092-t1-t2** (refinada, **fase 2 pendente**) — T1/T2 do `TODO.md`: corrigir o flake order-dependente do `SeedScriptsTest` (`TestDatabase.team_row` ganha filtro `user_id`, 9 chamadores em 3 arquivos) e registrar em `docs/draft-backlog.md` a destrutividade do `rake db:setup`/`TestDatabase.setup!` (TRUNCATE de `team_pokemons`/`team_pokemon_progress` a cada execução) + proposta de remediação (guard/once — não implementar agora, D1); cherry-pick de `dfbe217` (cassetes VCR, fora de `main`) como passo 1; `> Reprodução: script`, `> E2E: nao`.
...
| 0083 | ui-polish — polimento visual home/team/battle (header gate, filtros, pills, pcard; visual-only, sem backend/regras/rotas) | Concluída | Done (Passos 1–4 `294cb55`/`cbd3653`/`45e2528`/`71cac6a` + ajustes S7 `70e000a`/`1dff881`/`c8635f6`/`91103f7`/`0f5302a`, suíte 1186/6305 lint 0, revisor Aprovado `reviews/review-2026-09-16T22-20-37-0083-rodada2.md` + mini-passe final `reviews/review-2026-09-16T22-42-04-0083-final.md`, validada em 2026-09-16 — S2 C1–C4+G1–G3+M1 ok, sem `nok`) |
...
| 0089 | limpeza-user-state-e2e — dívida técnica em 2 blocos numa sessão só (D1): **(A)** remover o estado morto `user_state` — apaga a migração de criação (`0036_add_user_state.sql`) e cria `0037_drop_user_state.sql` (`DROP TABLE IF EXISTS`, idempotente, não destrutiva — sem `TRUNCATE`), `rm lib/user_state_repository.rb` (leitura morta, zero chamadores) e a escrita vestigial (`server.rb:599` → `journey_service#mark_started*` → upsert), além de todo o scaffolding de teste (`clear_user_state!`, helper `start_journey` + 4 callers, `test/user_state_repository_test.rb`, 2 testes vacuosos da flag + `JourneyMarkWhenFullTest`) — o gate **fica** e continua **derivado do time** (`lib/journey_service.rb:8`, `team >= 6`; decisão da sessão 0052, `REQUIREMENTS.md:638-643`); docs sincronizados no mesmo commit da remoção (GDD/REQUIREMENTS/draft/`docs/5F-decisoes-pendentes.md` — D7); **(B)** reparar os 4 e2e vermelhos (`e2e/specs/battle-log.spec.ts:51,77,101,112`) trocando `buildTeamOfSix` (bug do `nth` sobre a lista que encolhe a cada add → chimchar = linha S = 470 > 450 barra o 6º add) pelo `buildBudgetTeam` já existente, sem tocar no orçamento 450 (`lib/team_budget.rb:8`); D5 (stub de PokeAPI/VCR) **anotado e dimensionado, não refinado** | Concluída | Done (Passos 1–8 `d57947c`/`d6f1ff3`/`e7bcaf3`/`34b07b0`/`a487875`/`48a6774`/`dfd3741`/`85f48af` + ajustes S7 `8cdc045`/`b38e149`/`4e19b47`/`5b36a24`/`e17aaf9`, suíte 1185/6332 lint 0, revisor Aprovado `reviews/review-2026-09-17T01-49-27-0089-rodada3.md` — rodada 3, validada em 2026-09-16 — S2 C1–C9+G1–G3+M1 ok, sem `nok`) |
| 0090 | cta-oob — CTA "Batalhar" (gate + hint) via OOB nas rotas que mexem no time (T5 do `TODO.md`): partial `views/_cta_slot.erb` (`#cta-slot`, D1=A, padrão `nav-badge`), OOB `outerHTML` em `POST`/`DELETE /team`, `POST /team/heal`, `POST /journey/restart`, `POST /mart/buy`/`sell`; asserts novos nos arquivos de rota (D3); pill S3 Faltam N (M1 emendado); `> Reprodução: manual`, `> E2E: nao` | Validação | **Concluída (Done)** — PR #2 mergeado em 2026-09-21 |
| 0091 | T3+T4 rodada 3 da 0089: reindentar `<p class="rewards">` + assert exato de XP em derrota | Validação | **Concluída (Done)** — PR #3 mergeado em 2026-09-21 |
| 0092 | T1+T2 do `TODO.md`: flake order-dependente do `SeedScriptsTest` (`team_row` filtra por `user_id`) + `db:setup` destrutivo documentado em `docs/draft-backlog.md` com proposta de remediação | Refinamento | Concluída em 2026-09-21, fase 2 pendente |
