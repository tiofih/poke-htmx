...
**Sessão 0059 (Q5 — remover em 1 clique, hardening htmx + OOB condicional) refinada em 2026-08-27** (fase 1 concluída — critérios C1–C5 e plano TDD fechados em `sessions/0059-q5-remover-em-1-clique.md`): `DELETE /team` via htmx remove em 1 request, idempotente, sem re-submit, hardening `hx-disabled-elt`/`hx-sync="closest form:replace"`/`hx-indicator="#team-view"` + `oob_pokemon_list` condicional só quando `starters_visible?` (`q.empty? && offset.zero? && !filter_active? && !sort_active?`, preservando badges `poke-cost` e evitando varredura de 27 destaques quando filtrado/paginado/ordenado); **Implementação pendente (fase 2):** 4 passos (C1 htmx exato, C2+C3 idempotência+hardening, C4 OOB condicional, C5 docs/manual). Depois da 0059: fila J2/J4/D4 e limitações técnicas (escritas não atômicas, race no add, `?as=`/CSRF, schema, vida zerada fora) — a critério do usuário.
...
| 0059 | Q5 — remover em 1 clique (hardening htmx + OOB condicional) — DELETE htmx em 1 request, idempotente, sem re-submit + OOB condicional só quando starters visíveis | Refinamento | Concluída (fase 1 — critérios C1–C5 e plano TDD em `sessions/0059-q5-remover-em-1-clique.md`, aguardando fase 2) |
