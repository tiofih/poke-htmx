...
**Sessão 0066 (pool-oponente-base-form — Onda 2 Economia #3) refinada em 2026-08-28** (fase 1 concluída — critérios C1–C10 + G1–G3 e plano TDD 3 passos fechados em `sessions/0066-pool-oponente-base-form.md`): pool do oponente filtrado **dentro de `OpponentGenerator`** via `base_checker` injetável (`->name{ api.base_form?(name) }`) + `generation_for ≤ player_generation` (`ceil(avg/2)` clamp 1..9, fail open se nil) + **varredura intercalada single pass** (`Parallelizer` lotes 24 + `ratings:` + `band` + `generation` juntos) + **orçamento fictício ≤450** (`TeamBudget.cost_for` acumulado após banda, antes de fallback) + **level = average + delta por banda** (F/D 0, C/B +1, A/S +2) + **fallback com remanescente do pool base sem repetição determinístico por seed**; `BattleService` só injeta `base_checker`/`generation`/`budget` (sem gateway novo, sem migração) + `RATING_SCAN_CAP 256` + `PokemonRatingCache` 7d preservados. **Próxima sessão:** 0066 em implementação (TDD 3 passos) → 0067 pedras+modais → 0068 resolver batalha → 0063 juice — a critério do usuário.
...
| 0066 | pool-oponente-base-form — Onda 2 Economia #3: pool só base-form + geração ≤ player_generation + orçamento fictício ≤450 + level average+delta por banda, via `base_checker` injetável em `OpponentGenerator` com varredura intercalada (single pass) lotes 24 + cache 7d + cap 256 + fallback base sem repetição | Refinamento concluído | — (fase 1 concluída em 2026-08-28; fase 2/3 pendentes — TDD 3 passos + revisão + validação) |
