...
**Sessão 0076 (open-design-modals-filters — Onda open-design, 5ª de 5, última da onda) refinada em 2026-09-09** — decisões B fatiar 0076a+0076b em arquivo único (sufixo-letra rejeitado: invisível ao `check_docs`), B modelo cheio, C overlay HÍBRIDO (`:target` + htmx, sem JS), B `data-od-id` contrato nas 4 superfícies, C limpeza sob demanda, A tags cor por tipo com dado real (**toca backend/presenter — quebra o invariante 0072–0075**, mínimo listado: 2 rotas fragmento + enrichment `types` + filtro `team`, sem migração/schema/gems/services), B `team=in|out` + dress ODS, B riscos D71/D86/D88/D94 como critérios; fase 2a (C1–C10+G1–G3+M1: rotas center/mart, overlay, gates, contrato, tags, team-filter, dress, curadoria, rede D86/D88) com legado intacto → validação 3a; fase 2b (C11–C14+G4–G5+M2: sem sakura, colisões sob demanda, juice do bloco, filtros 100% ODS) → validação 3b que fecha a onda. **Próxima sessão:** **0077 escritas atômicas** → 0078 CSRF → 0079 respiro.
