...
- **`rake db:setup` / `TestDatabase.setup!` truncam `team_pokemons` e `team_pokemon_progress` em toda execução** — as migrações `0003`/`0007` rodam `TRUNCATE team_pokemons CASCADE` e o setup reaplica **todas** as migrações a cada execução (sem ledger de executadas; `Rakefile:16-24`, `test/test_helper.rb:41-55`), então rodar `./scripts/rake db:setup` (ou `TestDatabase.setup!`) **apaga os times de todos os usuários** — inclusive o time seedeado do pokedex. Hoje não estava documentado em lugar nenhum (evidência complementar em **§5.E**). **Proposta de remediação registrada (não implementada):** guard/once para migrações destrutivas — tracker de migrações já aplicadas (ledger no banco) para que TRUNCATEs só rodem na criação real do schema, ou gate explícito (ex.: `ALLOW_DESTRUCTIVE_MIGRATIONS=1`) nos arquivos 0003/0007; implementação é escopo futuro (D1 da sessão 0092, decisão de não implementar agora).
...
- **`PokeApiStub` — adoção real para o VCR parar de acumular episódios (anotado na 0089 em 2026-09-16; dimensionado, **NÃO refinado** — ideia não abre escopo)** — o `PokeApiStub` (`test/test_helper.rb:227+`) **já tem ~13 helpers** (`with_gateway`, `with_find`, `with_detail`, `with_all_names`, `with_base_forms`, `with_moves_for`, `with_move`, `with_available_move_names`, `with_type`, `with_next_evolutions`, `with_stone_evolutions`, `with_learnable_moves`, `with_generation`, `with_pokemon_names_by_type`, `with_type_names`) e o fake (`test/poke_api_fake.rb:5-97`) cobre `find`/`detail`/`paginate`/`move`/`moves_for`/`available_move_names`/`next_evolutions`/`stone_evolutions`/`learnable_moves`/`base_form?`/`evolution_restricted?`/`generation_for`/`pokemon_names_by_type` — **correção de premissa**: o stub **não** tem só `with_gateway/with_find/with_detail` e o fake **cobre** evolução (não há `evolution_chain` cru a emular). O gargalo é a **adoção**: dos **76** arquivos de teste, os maiores ainda misturam hits reais com helpers (ex.: `test/team_routes_test.rb` ~50 usos de stub + cassettes); com `record: :new_episodes` (`test/vcr_setup.rb:13,27`), cada run **cresce** os cassettes versionados — hoje **43** cassettes somando **341 MB**, o maior com **208 MB** (`test/cassettes/ServerTeamRemoveQ5Test/test_oob_conditional_skips_starters_when_filtered_or_paginated.yml`). Fatia candidata (a decidir em refinamento próprio): trocar hits reais por helpers **por arquivo**, começando pelo `ServerTeamRemoveQ5Test` (+ o stub de `load_starters`/`STARTER_SLUGS` já anotado acima) e medindo pelo tamanho de `test/cassettes` por run.
- **e2e `e2e/specs/battle-log.spec.ts` vermelho pré-existente (anotado 2026-09-15, NÃO faz parte da 0087; dimensionado na 0089 em 2026-09-16)** — `buildTeamOfSix` (região `:17`) trava no 6º add com "Orçamento insuficiente", derrubando 4 testes do arquivo (incluindo a expectativa de badge). Os dois testes adicionados pela 0087 passam. Pré-existente, não é regressão da 0087; ajustar o helper (orçamento/ordem dos adds do time fixo) antes de reusar o arquivo como gate. **Diagnóstico fechado no refinamento da 0089:** o helper usa `nth(i)` sobre a lista **que encolhe** a cada add (a carta vira "já está no time") → clica as cartas 1,3,5,7,9,11 = bulbasaur, squirtle, cyndaquil, treecko, mudkip, chimchar; chimchar → infernape = linha S (120) → 350+120 = 470 > 450 e o 6º add é barrado. O orçamento **não** está stale (fonte única `lib/team_budget.rb:8`); a correção é o `buildBudgetTeam` (`:157-164`, clica por nome exato) já usado pelos testes da 0088 — **reparo em execução na 0089 (Bloco B, Passo 6)**.
