...
| 23 | Fase Eco — moeda (Eco-1), Poke Center (Eco-2), Poke Mart (Eco-3), itens em batalha (Eco-4) | **Eco-1 Done (sessão 0027, em fase 2 — suíte 386/1216, lint 0, aguardando validação)**: tabela `wallet` + `WalletRepository` (balance/grant upsert) + `RewardRule#money_for` (win 100/draw 50/lose 40) concedido no hook `:finished` + aviso no `battle.erb`. **Eco-2 (Poke Center), Eco-3 (Poke Mart), Eco-4 (itens em batalha)** — pendentes |
