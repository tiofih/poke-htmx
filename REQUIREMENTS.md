...
- [x] **Bug: oponente SEMPRE o mesmo por usuário (anotado 2026-08-25, confirmado no
      ...
      dela é determinística). **Correção (0049):** gerar oponente **novo a cada
      confronto** via dependência injetável `opponent_rng` (default `-> { Random.new }`)
      em `build_opponent`, mantendo a dificuldade ajustada ao time atual pela banda.
