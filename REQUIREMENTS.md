...
- [x] **Performance do `GET /battle` (anotado 2026-08-23, durante validação da 0040):**
      ...
      **RESOLVIDA na sessão 0050 (Done, validada em 2026-08-25):** varredura da banda em
      **lotes paralelos** (`ratings:` no `OpponentGenerator`) + **cap de varredura**
      (`max_candidates:` 256) com fallback puro + **`PokemonRatingCache`** persistente
      (keyed por nome) no `BattleService` + **Ajuste S3 (C4-b): escrita do cache HTTP
...
- [x] **Bug: oponente SEMPRE o mesmo por usuário (anotado 2026-08-25, confirmado no
      ...
      `/battle`. Causa: `BattleService#build_opponent` (`lib/battle_service.rb:64`) usava
      `Random.new(user_id.sum)` — **seed determinístico por usuário** — então o
      `OpponentGenerator` produzia sempre o mesmo oponente para o mesmo usuário, a cada
      `prepare`, independente do time atual/nível (a banda varia, mas a escolha dentro
      dela é determinística). **Correção (0049):** oponente **novo a cada novo confronto**
      via dependência injetável `opponent_rng` (default `-> { Random.new }`) em
      `build_opponent` + **máquina de estado da batalha ativa** (`prepare` reusa por
      estado — preparada re-deriva o time preservando o oponente, em andamento/finalizada
      preserva o engine; "Novo confronto" via `POST /battle/new`; add/remove/move
      invalidam), mantendo a dificuldade ajustada ao time atual pela banda.
- [x] **Bug: remover Pokémon com itens equipados perde os itens (anotado 2026-08-25,
      durante a preparação do playtest 2 — fora de sessão, RNF-04; **RESOLVIDO na sessão
      0052, 2026-08-25**):** ao **remover um Pokémon com itens/seguráveis equipados, todos
      os itens somem** do estoque. `assigned_item`/`held_item` vivem na própria linha do
      `team_pokemons`; `TeamRepository#remove` (`lib/team_repository.rb:188`) faz o `DELETE`
      sem **repor** os itens ao inventário, e o handler `remove_team_member`
...
- [x] **Bug: remover do time às vezes exige clicar 2x (anotado 2026-08-25; correção na
       sessão 0053, 2026-08-25 — **persiste** até 0058; **RESOLVIDO na sessão 0059, 2026-08-27**):** por vezes era preciso clicar 2x no botão "Remover do time"
       ...
       **Correção definitiva (0059):** hardening `hx-disabled-elt="this"` + `hx-sync="closest form:replace"` + `hx-indicator="#team-view"` no form + `oob_pokemon_list` condicional só quando `starters_visible?` (`q.empty? && offset.zero? && !filter_active? && !sort_active?`) — 1 request remove com OOB de ambos fragmentos quando visível, idempotente no 2º clique, sem varrer 27 starters quando filtrado/paginado/ordenado, preservando badges `poke-cost`.
...
- [x] **Bug/UX: busca só acha formas base (anotado 2026-08-25, no QA; **RESOLVIDO na
      ...
      pool de busca via `reject STARTER_SLUGS`). **Decisão (0053):** a busca segue só
      formas base, mas quando o termo só casa com não-base/starter mostra **aviso
      informativo** apontando a evolução base (via `evolutions.first`) ou os destaques
      iniciais — `search_hint` no `server.rb`.
- [ ] **Ideias novas (anotado 2026-08-25, fora do fluxo — RNF-04):** (1) **Poke
      ...
      Onda 2 Economia — implementada e validada em 2026-08-31, suíte 983/3819 lint 0)**:
      as **6 pedras clássicas** entraram com **oferta
...
> **0076 open-design-modals-filters Done** (onda open-design fechada em 2026-09-09 —
> suíte 1057/4900, lint 0). **Resíduo open-design fatiado por tela (refinado em
> 2026-09-09, numeração desliza):** **0077 open-design-home-1a1** (`home-team.html` →
> miolo center/mart + manage/evolução + list-item/detalhe) → **0078
> open-design-battle-1a1** (`battle.html` + end-states + results-desktop → fim de
> `battle.erb`, motor só leitura) → **0079 open-design-history-1a1** (curadoria fina
> 1:1 do `history.html`).
> Depois volta a **Onda 3 Estabilidade (deslizada):** **0080 escritas atômicas** →
> **0081 CSRF** → **0082 respiro**. Referência em `SESSIONS.md` (nota da 0071 +
> refinamento do resíduo em 2026-09-09).
