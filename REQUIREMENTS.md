...
| 25 | **Próximas sessões (ordem fechada em 2026-08-18)** — 1. **Respiro 2** (extrair `BattleService`/`TeamService` + split `server_test.rb`); 2. **D1 parcial** (nível de aprendizado de golpes); 3. **J1** (seleção inicial); 4. **JN-2** (golpes em lista); 5. **J3** (ranking S–F); 6. **JN-1** (telas próprias, fim do empilhamento) — depois organizar o resto (JN-3, JN-4, JN-5, J2, J4, D4) | **Respiro 2 em implementação (sessão 0033, TDD concluído em 2026-08-19 — suíte 559/1696, lint 0, grep `rubocop:` em `server.rb` → 0; aguardando validação)**: `BattleService` (`prepare`/`advance`, módulos Preparation/Finalization) + `TeamService` (`manage_data`/`save_moves`/`assign_item`/`assign_held_item`) — services usam providers lambda p/ a gateway (testes trocam instância em runtime) e compartilham as mesmas instâncias de repositórios; handlers de batalha/time/itens ficam **thin**; `test/server_test.rb` (1925 linhas) fatiado em 7 arquivos por área + `battle_test_helpers.rb`. Próxima: **D1 parcial** |
