...
- **Impacto:** `TeamService` (validação de disponibilidade no `assign_item`/
  `assign_held_item` — contar quantos pokes já estão equipados com o mesmo nome vs.
  quantidade do estoque; aviso amigável e sem gravar quando esgota) + `team_manage.erb`
  (no option, indicar **em uso por outro poke** e/ou **quantidade livre** =
  estoque − já equipados) + testes de service/rotas do manage. Cruza com Eco-3/Eco-4.
- **Aberto:** equipar **debita** o estoque (reserva) ou apenas limita (estoque continua
  como "comprado", equipados contam contra o limite)? O que acontece quando o poke
  equipado usa o item na batalha (o débito do `debit_used_items` já existe) ou
  desequipa — a unidade volta a ficar livre?
- **Aguarda sessão (RNF-04).**
