...
| **8** | **Eco-4 — Itens em batalha** | Consumíveis (poções) como **ação automática** no motor, depois aberta ao jogador (12); seguráveis escopo simples (13); estratégia do time **selecionável** (12). **Eco-4-A (sessão 0030, fase 2 — suíte 493/1525, lint 0, aguardando validação):** poções como ação **automática** via `ItemUsePolicy` determinística (1ª parte da decisão 12) + ação `:item` no half-FSM (6.1) + débito do inventário no `POST /battle/play`. Pendentes: Eco-4-B (estratégia **selecionável** — 2ª parte da 12), seguráveis (13). | 6/7 (itens), 3 (estratégia/engine) |
...
| **Eco-4 — Itens em batalha** | Consumíveis (poções restauram HP **durante** a batalha — ação não-ofensiva no motor), seguráveis (hold items modulam stats/passivos via Strategy). **Eco-4-A implementado na sessão 0030 (fase 2, suíte 493/1525, lint 0 — aguardando validação):** `Item#heal_amount` + `BattlePokemon#heal` (clamp) + `ItemUsePolicy` automática + ação `:item` no half-FSM + `InventoryRepository#use` + débito no `POST /battle/play`. Eco-4-B (estratégia selecionável) e seguráveis: pendentes (próximas sessões). | `BattleEngine` ganha tipos de ação além do ataque → **confirma o half-FSM** (ação = evento com transição); hold items via **Strategy/Decorator** sobre `BattlePokemon` (stats). |
...
- **FAZER (half-FSM):** estado **terminal explícito** da batalha — hoje `finished?`/
  ...
  - **Eco-4 (items em batalha):** poção/consumível é **ação entre turnos** —
    ...
    **Aplicado na sessão 0030 (Eco-4-A):** ação `:item` via `apply_heal` (módulo
    `BattleItemActions`) + política injetável `ItemUsePolicy` — `BattleEngine#act`
    não ramificou; `finished?`/`winner` continuam por inspeção.
...
| 12 | Consumível em batalha (Eco-4) | **Automático no início** (estratégia decide); depois abrir para o jogador escolher — **o mesmo vale para a estratégia do time** (estratégias selecionáveis). **1ª parte (automático) atendida na sessão 0030 (Eco-4-A): `ItemUsePolicy` determinística injetável; 2ª parte (selecionável) pendente — Eco-4-B.** |
