...
| **7** | **Eco-3 — Poke Mart (catálogo + inventário)** | `Item` (Value Object), catálogo estático, compra com dinheiro, `InventoryRepository`, fragmentos htmx. **Implementado na sessão 0029 (2026-08-14 — suíte 459/1432, lint 0, aguardando validação):** `Item` + `ItemCatalog` (potion 20 / super-potion 50 / hyper-potion 100); tabela `inventory` (PK user_id+item_name) + `InventoryRepository` (all/add upsert/count); `MartService` (compra via `spend`); `POST /mart/buy` + bloco Poke Mart no `team.erb`; seed `saldo_inicial` (200). | 5 (saldo) |
