...
| **5** | **Eco-1 — Moeda pós-batalha** | Tabela `wallet`; `RewardRule` passa a conceder **XP + dinheiro** no `:finished` (decisão 10 — moeda ao final da batalha como um todo). **Implementada na sessão 0027 (fase 2 — suíte 386/1216, lint 0, aguardando validação):** tabela `wallet` (user_id PK) + `WalletRepository` (`balance`/`grant` upsert) + `RewardRule#money_for` (win 100/draw 50/lose 40, injetável) + `grant_finished_money` no hook `:finished` + aviso no `battle.erb`. | 3 (mesmo hook), 4 (resultado persistido) |
...
| 10 | Eco: quando conceder moeda | **Ao final da batalha como um todo** — XP + moeda conjugados no hook `:finished` (`RewardRule`) — **atendida na sessão 0027** |
