...
| **Eco-2 — Poke Center** | Serviço de recuperação: custo baseado em HP faltante (e/ou reavivar?); rota htmx `POST /team/heal` + fragmento; cobra do saldo. **Feito na sessão 0028 (fase 2, 2026-08-14):** HP persistente + carryover; `HealCostPolicy`; `spend`; `HealService`; `POST /team/heal` + Poke Center no `team.erb`. | `HealService` (uso de caso); cálculos puros (`HealCostPolicy`); reforça persistir HP/status do time (D2 já persiste). |
...
| 11 | Poke Center: custo | **Proporcional ao HP faltante** (`HealCostPolicy`) — **atendida na sessão 0028 (2026-08-14)**: `DEFAULT_COST_PER_HP 0.5` (balanceamento aberto, calibra win 100/draw 50/lose 40) |
